...
def get_api():
    if os.path.isfile('api_key'):                                   # checks api_key file is there are not
        ...
        try:
            key = input('Enter your Virustotal API key: ')          # If no api_key file,then get key from user
            if check_api(key):                                      # checks api key is valid or not (True or False)
                while True:
                    if check_api(key):                              # checks api key is valid or not (True or False)
                        print('Invalid API Key')
                        key = input('Enter Valid Virustotal API key: ')  # If api_key is invalid,then get key from user
                    else:
                        break
